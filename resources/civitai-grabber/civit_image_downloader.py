...
MODEL_VERSIONS_API_URL: str = "https://civitai.com/api/v1/model-versions"
...
# Civitai occasionally returns 408 for cursor pagination pages that are slow
# server-side. Treat it like 429/5xx: temporary, worth retrying with backoff.
...
class CivitaiDownloader:
    ...
    def _validate_next_page(self, url: Optional[str]) -> Optional[str]:
        ...
        Keeps withMeta=true on cursor pagination so subsequent pages include
        generation metadata too.
        """
        if not url:
            return None
        try:
            ...
            if p.path.startswith("/api/v1/images") and query.get("withMeta") != "true":
                query["withMeta"] = "true"
    ...
    def _image_api_url(self, **params: Any) -> str:
        """Builds CivitAI image API URLs with an explicit max page size and metadata.
        ...
        withMeta=true is required by the current API to include generation
        metadata; without it many otherwise valid items return meta=null.
        """
        query_params = {"limit": IMAGE_API_LIMIT, "withMeta": "true"}
    ...
    async def _write_meta_data(self, meta: Optional[Dict[str, Any]], base_output_path_no_ext: str, image_id: str, username: Optional[str], base_model: Optional[str] = None, api_item: Optional[Dict[str, Any]] = None) -> Tuple[bool, Optional[str]]:
        ...
        Args:
            ...
            api_item: Original Civitai API item, used for fallback metadata when `meta` is null
        """
        ...
        if not meta or all(str(v).strip() == '' for v in meta.values()):
             fallback_model = base_model or ((api_item or {}).get("baseModel") if isinstance(api_item, dict) else None)
             if fallback_model:
                 meta_filename_suffix = "_meta.txt"
                 model_version_ids = (api_item or {}).get("modelVersionIds", []) if isinstance(api_item, dict) else []
                 if isinstance(model_version_ids, list):
                     model_version_ids = ", ".join(str(v) for v in model_version_ids)
                 content_lines = [
                     f"Model: {fallback_model}",
                     "Metadata: unavailable from Civitai API",
                     f"Image ID: {image_id}",
                     f"Username: {username}",
                     f"Base model: {fallback_model}",
                     f"Model version IDs: {model_version_ids}",
                     f"Post ID: {(api_item or {}).get('postId', '') if isinstance(api_item, dict) else ''}",
                     f"Created at: {(api_item or {}).get('createdAt', '') if isinstance(api_item, dict) else ''}",
                     f"Width: {(api_item or {}).get('width', '') if isinstance(api_item, dict) else ''}",
                     f"Height: {(api_item or {}).get('height', '') if isinstance(api_item, dict) else ''}",
                     f"NSFW level: {(api_item or {}).get('nsfwLevel', '') if isinstance(api_item, dict) else ''}",
                     f"Image URL: {(api_item or {}).get('url', '') if isinstance(api_item, dict) else ''}",
                     f"URL: {CIVITAI_WEB_URL}/images/{image_id}?username={username}",
                 ]
             else:
    ...
    async def _fetch_api_page(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Handle non-retryable client errors (4xx). Retryable 4xx such as
            # 408/429 fall through to raise_for_status(), which triggers tenacity.
            if 400 <= response.status_code < 500 and response.status_code not in RETRYABLE_STATUS_CODES:
    ...
    @retry(
        stop=stop_after_attempt(CURRENT_API_RETRY_COUNT),
        wait=wait_random_exponential(multiplier=1, max=DEFAULT_API_RETRY_MAX_WAIT),
        retry=retry_if_exception(should_retry_exception),
        before_sleep=before_sleep_log(retry_logger, logging.WARNING),
        retry_error_callback=return_none_after_retry
    )
    async def _fetch_model_version_details(self, model_version_id: str) -> Optional[Dict[str, Any]]:
        """Fetch one model-version record so mode 4 folders can use readable names."""
        url = f"{MODEL_VERSIONS_API_URL}/{quote(str(model_version_id), safe='')}"
        client = await self._get_client()
        try:
            async with self.semaphore:
                response = await client.get(url)
            if 400 <= response.status_code < 500 and response.status_code not in RETRYABLE_STATUS_CODES:
                if response.status_code == 404:
                    self.logger.warning(f"Model version not found: {model_version_id}")
                    return {"error": "model version not found"}
                self.logger.error(f"Model version details Client Error {response.status_code} for {url}")
                return None
            response.raise_for_status()
            return response.json()
        except json.JSONDecodeError as e:
            self.logger.error(f"JSON Decode Error model version details {url}: {e}")
            return None
        except httpx.HTTPStatusError as e:
            if is_retryable_http_status(e):
                raise
            self.logger.error(f"HTTP error model version details {url}: Status {e.response.status_code}")
            return None
        except httpx.RequestError as e:
            if isinstance(e, RETRYABLE_EXCEPTIONS):
                raise
            self.logger.error(f"Network error model version details {url}: {e}")
            return None

    async def _model_version_folder_name(self, model_version_id: str) -> str:
        """Return a readable folder name like 'Model Name - Version Name'."""
        fallback = f"modelVersion_{model_version_id}"
        data = await self._fetch_model_version_details(model_version_id)
        if not data or data.get("error"):
            return fallback
        model = data.get("model") if isinstance(data.get("model"), dict) else {}
        model_name = (model.get("name") or data.get("modelName") or "").strip()
        version_name = (data.get("name") or "").strip()
        if not model_name and not version_name:
            return fallback
        label = model_name or f"modelVersion_{model_version_id}"
        if version_name and version_name.lower() not in label.lower():
            label = f"{label} {version_name}"
        cleaned = self._clean_path_component(label)
    ...
    async def _handle_single_download(self, item: Dict[str, Any], target_dir: str, current_tag: Optional[str] = None, parent_result_key: Optional[str] = None, model_id: Optional[int] = None) -> None:
        ...
        if success and final_image_path:
             ...
             await self._write_meta_data(meta, base_path_no_ext, str(image_id), username, base_model=model_name_for_meta, api_item=item)
             if not meta or all(str(v).strip() == '' for v in meta.values()):
    ...
    async def _run_paginated_download(self, initial_url: str, target_dir: str, mode_specific_info: Optional[Dict] = None, parent_result_key: Optional[str] = None, model_id: Optional[int] = None, deep_scan_pass: Optional[str] = None, continuation_pass: Optional[str] = None) -> int:
        ...
        while url:
             ...
             if page_data is not None: # Process page if fetch succeeded and wasn't a "Not Found" error handled above
                 ...
                 else: identifier_status = 'Completed (Partial - Fetch Error on Subsequent Page)'; identifier_reason = fetch_fail_reason
    ...
    async def run(self) -> None:
        ...
        # Map mode number/type to the base folder name. Empty string means downloads go
        # straight into output_dir/ instead of a per-mode parent folder, e.g.
        # output_dir/<username>/ and output_dir/model_<id>/ rather than
        # output_dir/Username_Search/<username>/ and output_dir/Model_ID_Search/model_<id>/.
        # _create_option_folder('') resolves to output_dir, so this is safe.
            ...
            if self.mode == '3': # Tag Search Special Handling
                ...
                for idt, ident in identifiers_to_process:
                     ...
                     if idt == 'username':
                         ...
                         target_dir = os.path.join(option_folder, await self._model_version_folder_name(ident))
